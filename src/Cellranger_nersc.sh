...
#SBATCH --time=110:00:00          	# Limit of wall clock time - how long the job will run (same as -t)
#SBATCH --nodes=1            		# Number of different nodes - could be an exact number or a range of nodes (same as -N)
#SBATCH --ntasks=1          		# Number of tasks - how many tasks (nodes) that you require (same as -n)
#SBATCH --cpus-per-task=100    		# Number of CPUs (or cores) per task (same as -c)
#SBATCH --mem=60G       	       	# Specify the real memory required per node. (20G)
#SBATCH --job-name CRcounts 	     	# You can give your job a name for easier identification (same as -J)
#SBATCH -A egl 
